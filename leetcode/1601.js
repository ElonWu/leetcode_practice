...
var maximumRequests = function (n, requests) {
  let transfer = new Array(n).fill(0);

  for (let i = 0; i < requests.length; i++) {
    const [from, to] = requests[i];

    if (!transfer[to]) transfer[to] = [[], []];
    transfer[to][0].push(from);

    if (!transfer[from]) transfer[from] = [[], []];
    transfer[from][1].push(to);
  }

  let set = new Set();
  for (let arr of transfer) {
    if (arr[0].length && arr[1].length) {
      for (let num of arr[0]) set.add(num);
    }
  }
