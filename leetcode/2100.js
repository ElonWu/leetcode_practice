...
var goodDaysToRobBank = function (security, time) {
  if (time === 0) return security.map((_, i) => i);

  let res = [];

  for (let index = time; index < security.length - time; index++) {
    if (security[index] === undefined) break;

    if (isGood(security, index, time)) {
      res.push(index);
    }
  }
...
const isGood = (security, index, time) => {
  for (let offset = 1; offset <= time; offset++) {
    if (
      security[index - offset] < security[index - offset + 1] ||
      security[index + offset] < security[index + offset - 1]
    ) {
      return false;
    }
