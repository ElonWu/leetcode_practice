...
var compareVersion = function (version1, version2) {
  ...
  while (oneStart < version1.length || twoStart < version2.length) {
    // 比较当前片段
    const res1 = findNext(version1, oneStart);
    const res2 = findNext(version2, twoStart);
    const res = compare(res1.version, res2.version);
    if (res !== 0) return res;
...
const findNext = (str, start) => {
  ...
  return {
    version: str.slice(start, end) || '0',
    next: Math.min(end + 1, str.length),
  };
};
...
console.log(compareVersion('1.001.001', '1.001.01'));
